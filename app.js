...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  var addTotal = sum(sum(a,b)[0],c)[0];
  var mulTotal = multiply(multiply(a,b)[0],c)[0];
  var addString = a.toString() + ' and ' + b + ' and ' + c + ' sum to ' + addTotal + '.';
  var mulString = 'The product of ' + a + ' and ' + b + ' and ' + c + ' is ' + mulTotal + '.';
  return [addTotal, mulTotal, addString, mulString];
}
...
function sumArray(sumArr) { //eslint-disable-line
  ...
  var arrayValsString = '';
  var outputString = '';
  for (var nums = 0; nums < sumArr.length; nums++){
    arrayValsString += sumArr[nums];
    if (nums < sumArr.length - 1){
      arrayValsString += ',';
    }
  }
...
function multiplyArray(multArr) { //eslint-disable-line
  ...
  for (var nums = 0; nums < multArr.length; nums++){
    arrayValsString += multArr[nums];
    if (nums < multArr.length - 1){
      arrayValsString += ',';
    }
  }

  total = multiply(multiply(multArr[0],multArr[1])[0], multArr[2])[0];
  answerString = 'The numbers ' + arrayValsString + ' have a product of ' + total + '.';

  return [total, answerString];
}
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  ...
  for (var nums = 0; nums < dynamicArray.length; nums++){
    arrayValsString += dynamicArray[nums];
    if (nums < dynamicArray.length - 1){
      arrayValsString += ',';
    }
  }
